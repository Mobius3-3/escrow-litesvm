...
mod tests {
    ...
    fn setup_escrow(program: &mut LiteSVM, payer: &Keypair, is_time_lock_required: bool) -> (Pubkey, Pubkey, Pubkey, Pubkey, Pubkey, Pubkey) {
        ...
        let make_ix = Instruction {
            ...
            data: crate::instruction::Make { deposit: 10, seed: 123_u64, receive: 10, is_time_lock_required: is_time_lock_required }.data(),
    ...
    speculate! {
        describe "escrow" {
            before {
                let is_time_lock_required = true;
                let (mut program, payer, taker) = setup();
                let (maker_address, mint_a, mint_b, maker_ata_a, escrow, vault) = setup_escrow(&mut program, &payer, is_time_lock_required);
                let (taker_ata_a, taker_ata_b, maker_ata_b) = setup_take(&mut program, &payer, &taker, &mint_a, &mint_b, &maker_address);
            }

            it "should create escrow and vault correctly" {
                ...
                assert_eq!(escrow_data.is_time_lock_required, is_time_lock_required);
            }

            #[ignore]
            it "should execute take correctly" {
                // Create and send take transaction
            ...
            // #[ignore]
            it "should fail when escrow is still locked" {
                // Try to call take_if_unlocked immediately (should fail)
                let take_ix = Instruction {
                    program_id: PROGRAM_ID,
                    accounts: crate::accounts::Take {
                        taker: taker.pubkey(),
            ...
            #[ignore]
            it "should execute take after 5 days when timelock enabled" {
            ...
            // #[ignore]
            it "should refund tokens and close vault" {
                ...
                let tx_result = program.send_transaction(transaction).unwrap();
